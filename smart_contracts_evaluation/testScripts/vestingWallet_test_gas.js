...
const timeLowerBound = 1;
const timeUpperBound = 10;
const durationLowerBound = 10;
const durationUpperBound = 20;

const testFolder = path.join(__dirname, `../testTraces/vestingWallet`);
...
helper.range(transactionCounts).forEach(l => {
  ...
  if(transactionName == 'release') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let startTime = helper.random(timeLowerBound, timeUpperBound+1);
      let duration = helper.random(durationLowerBound, durationUpperBound+1);
      let text = `release,constructor,,accounts[${beneficiaryIndex}] ${startTime} ${duration},,,false\nrelease,release,instance,,,,false\nrelease,increase,time,time.duration.seconds[${1}],,,false\nrelease,release,instance,,,,true\n`;
