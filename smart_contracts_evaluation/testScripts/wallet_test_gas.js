...
const testFolder = path.join(__dirname, `../testTraces/wallet`);
...
helper.range(transactionCounts).forEach(l => {
  ...
  if(transactionName == 'transfer') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let text = `transfer,constructor,,,${ownerIndex},,false\ntransfer,mint,instance,accounts[${mintAccountIndex}] ${mintAmount},${ownerIndex},,false\ntransfer,transfer,instance,accounts[${mintAccountIndex}] accounts[${transferAccountIndex}] ${transferAmount_1},${mintAccountIndex},,false\ntransfer,transfer,instance,accounts[${mintAccountIndex}] accounts[${transferAccountIndex}] ${transferAmount_2},${mintAccountIndex},,true\n`;
      if(!fs.existsSync(path.join(transactionFolderPath, fileName))) {
        console.log('generating new tracefiles ...');
        fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
          if (err) throw err;
