...
function runTests(transactionCounts, transactionFolders, testFolder, contractName, min_version) {
  ...
  contract(`${contractName}`, accounts => {
    range(transactionCounts).forEach(async(l) => {
        ...
        it(`Testing ${contractName}.${transactionName} gas consumption`, async() => {
              ...
              if(line != '') {
                ...
                // call transaction
                  ...
                  try {
                    ...
                    console.log(`Transaction reverted (test ${i}, line ${j}): ${e.reason || e.message}`);
                    fromAccountIndex = -1;
                    valueTran = -1;
                    continue;
                  }
          ...
          var successCount = resultArr.length;
          console.log(`Successful target transactions: ${successCount} out of total across ${transactionCount} trace files`);
          if(aggregationMethod == 'average' && successCount > 0) {
            gasUsedAgg =  resultTotal / successCount;
            gasUsedOpAgg = gasUsedOpTotal / successCount;
            gasUsedMemReadAgg = gasUsedMemReadTotal / successCount;
            gasUsedMemWriteAgg = gasUsedMemWriteTotal / successCount;
            gasUsedMemAgg_others = gasUsedMemTotal_others / successCount;
            gasUsedStorageAgg = gasUsedStorageTotal / successCount;
            gasUsedComputeAgg = gasUsedComputeTotal / successCount;
            storageOverheadAgg_read = storageOverheadTotal_read / successCount;
            storageOverheadAgg_write = storageOverheadTotal_write / successCount;
            readGasCostAgg = readGasCostTotal / successCount;
            writeGasCostAgg = writeGasCostTotal / successCount;
            sload_2100_slotAgg = sload_2100_slotTotal / successCount;
            sload_100_slotAgg = sload_100_slotTotal / successCount;
            sstore_22100_slotAgg = sstore_22100_slotTotal / successCount;
            sstore_20000_slotAgg = sstore_20000_slotTotal / successCount;
            sstore_5000_slotAgg = sstore_5000_slotTotal / successCount;
            sstore_2900_slotAgg = sstore_2900_slotTotal / successCount;
            sstore_100_slotAgg = sstore_100_slotTotal / successCount;
            storage_over_opAgg = round(storage_over_opTotal / successCount);
            read_over_opAgg = round(read_over_opTotal / successCount);
            write_over_opAgg = round(write_over_opTotal / successCount);
            read_over_writeAgg = round(read_over_writeTotal / successCount);
          ...
          fs.writeFileSync(path.join(transactionFolderPath, `/opcode_${min_version}.txt`), opcodeData);
        }) 
      })
