...
const testFolder = path.join(__dirname, `../testTraces/ltcSwapAsset`);
...
helper.range(transactionCounts).forEach(l => {
  ...
  if(transactionName == 'increaseAllowance') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let increaseAllowanceAmount_1 = helper.random(lowerBound, upperBound+1);
      let increaseAllowanceAmount_2 = helper.random(lowerBound, upperBound+1);
      let text = `increaseAllowance,constructor,,,${owner},,false\nincreaseAllowance,approve,instance,accounts[${approveAccountIndex}] ${approveAmount},${owner},,false\nincreaseAllowance,increaseAllowance,instance,accounts[${approveAccountIndex}] ${increaseAllowanceAmount_1},${owner},,false\nncreaseAllowance,increaseAllowance,instance,accounts[${approveAccountIndex}] ${increaseAllowanceAmount_2},${owner},,true\n`;
      if(!fs.existsSync(path.join(transactionFolderPath, fileName))) {
        console.log('generating new tracefiles ...');
        fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
          if (err) throw err;
  ...
  if(transactionName == 'decreaseAllowance') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let text = `decreaseAllowance,constructor,,,${owner},,false\ndecreaseAllowance,approve,instance,accounts[${approveAccountIndex}] ${approveAmount},${owner},,false\ndecreaseAllowance,decreaseAllowance,instance,accounts[${approveAccountIndex}] ${decreaseAllowanceAmount_1},${owner},,false\ndecreaseAllowance,decreaseAllowance,instance,accounts[${approveAccountIndex}] ${decreaseAllowanceAmount_2},${owner},,true\n`;
      if(!fs.existsSync(path.join(transactionFolderPath, fileName))) {
        console.log('generating new tracefiles ...');
        fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
          if (err) throw err;
  ...
  if(transactionName == 'changeDCRMOwner') {
    tracefileCount = transactionCount;
    helper.range(tracefileCount).forEach(testFileIndex => {
      let fileName = `${transactionName}_${testFileIndex}.txt`;
      let owner = helper.random(0, deployAccountCount);
      ...
      let height = 1;
      let text = `changeDCRMOwner,constructor,,,${owner},,false\nchangeDCRMOwner,changeDCRMOwner,instance,accounts[${newOwnerIndex_1}],${owner},,false\nnchangeDCRMOwner,advanceBlock,time,${height},,,false\nchangeDCRMOwner,changeDCRMOwner,instance,accounts[${newOwnerIndex_2}],${newOwnerIndex_1},,true\n`;
      if(!fs.existsSync(path.join(transactionFolderPath, fileName))) {
        console.log('generating new tracefiles ...');
        fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
          if (err) throw err;
