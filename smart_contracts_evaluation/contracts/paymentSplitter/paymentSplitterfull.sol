contract PaymentSplitter {
  ...
  OwnerTuple owner;
  mapping(address=>TotalReleaseOfTuple) totalReleaseOf;
  ...
  function updateReleasedOnInsertTotalReleaseOf_r7(address p,uint s) private    {
      released[p] = ReleasedTuple(s,true);
  }
  function updateReleasedOnIncrementTotalReleaseOf_r7(address p,int s) private    {
      int _delta = int(s);
      uint newValue = updateuintByint(totalReleaseOf[p].n,_delta);
      updateReleasedOnInsertTotalReleaseOf_r7(p,newValue);
  }
