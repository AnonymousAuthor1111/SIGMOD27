contract Ballot {
  ...
  struct EffectivedelegatorTuple {
    address delegator;
    bool _valid;
  }
  ...
  mapping(address=>EffectivedelegatorTuple) effectivedelegator;
  ...
  function updateEffectiveVoteOnInsertEffectivedelegator_r15(address d,address v) private    {
      EffectivedelegatorTuple memory toDelete = effectivedelegator[d];
      if(toDelete._valid==true) {
        updateEffectiveVoteOnDeleteEffectivedelegator_r15(d,toDelete.delegator);
      }
  ...
  function updateEffectivedelegatorOnInsertEffectiveDelegate_r4(address s,address to) private    {
      ...
      effectivedelegator[to] = EffectivedelegatorTuple(s,true);
  }
  ...
  function updateEffectiveVoteOnDeleteEffectiveVote_r15(address d,uint p) private    {
      address v = effectivedelegator[d].delegator;
      if(v!=address(0) && p!=0) {
