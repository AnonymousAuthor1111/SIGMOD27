special_keys = {'msgSender','now','this'}
        ...
        for name in files:
            ...
            with open(datalog_file,'r') as dl:
                for l in dl:
                    ...
                    # Collect non-transaction relations initialized by constructor rules.
                    # These may not appear in the dependency graph but still need materialization.
                    elif ':-' in l and 'constructor(' in l:
                        head = l.split('(')[0].strip()
                        if head and not head.startswith('.'):
                            constructor_heads.add(head)
            ...
            # Add constructor-initialized relations that are not txn heads, special keys, or functions
            constructor_heads = constructor_heads - txn_head_all - special_keys - set(calculate_on_demand)
            constructor_heads = {h for h in constructor_heads if not h.startswith('recv_')}
            print('\n\nconstructor_heads (to materialize)')
            pprint(constructor_heads)

            full_set = direct_dependency_set_all.union(set(public_relation_readonly)).union(constructor_heads)
            visited_set = set(full_set)
            queue = list(full_set)
            while True:
                if len(queue) == 0:
                    break
                ...
                for pred in G.predecessors(to_visit):
                        ...
                        if not (pred in txn_head_all):
                            full_set.add(pred)
                            if pred not in visited_set: # avoid infinite loop from self-edges (e.g. effectiveVote -> effectiveVote)
