    ...
    for root, dirs, files in os.walk("./view-materialization/relation-dependencies", topdown=False):
        for name in files:
            ...
            for min_set in minimal_all:
                ...
                # special handling for voting where "_" appears in the body relation's primary key && requires iterating the whole body relation wins table
                if name_normalize == 'voting':
                    for rel in ['hasWinner', 'winningProposal']:
                        if rel in func_set:
                            func_set.discard(rel)
                            min_set.add(rel)
