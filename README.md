# Artifact for Verifying OCELOT

## Experiment Setup
1. Download and unzip the compressed file `OCELOT.zip`, or git clone the current repo
3. Entering the folder `OCELOT`, this is the full project folder
...
### Run one contract
1. Generate all Solidity contract versions from scratch
```
./run_contrach.sh [contract name]
```
2. Open Ganache: ```ganache -a 10 -p 8545 --logging.debug```
3. Test all Solidity contract versions
```
cd smart_contracts_evaluation
./test.sh [contract name]
```
